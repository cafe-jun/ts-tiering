...
아래 4개가 모두 채워지면 Phase 1 완료. (3번은 2026-08-11 에 재정의했다 — 아래 참고)

1. ✅ 합성 데이터 1억 건을 Parquet으로 S3에 적재할 수 있다
2. ✅ 파티셔닝 스킴 3종에 대한 **스캔량 / 쿼리 지연 / 압축률 비교표**가 있다
3. ⚠️ ~~DuckDB와 Athena 양쪽에서 같은 쿼리를 돌린 **비교 결과**가 있다~~
   → **재정의 (2026-08-11):** AWS 가 이 프로젝트의 범위 밖으로 확정되어 Athena 비교는
   수행하지 않는다. 대신 그 질문의 핵심이었던 **"카탈로그가 붙으면 결론이 바뀌는가"**를
   로컬에서 답했다 ([W6 보강](benchmark/w6-manifest-vs-glob.md)) — 바뀐다.
   **관리형 엔진과의 비교 자체는 미검증으로 남는다.**
...
### W8 — Athena 비교 (실 AWS) ⛔ 수행하지 않음

- ~~실제 S3 + Glue Catalog + Athena~~
- ~~같은 쿼리 3종, 스캔 바이트와 비용 비교~~

**AWS 를 쓰지 않기로 확정했다 (2026-08-11).** 이 프로젝트는 로컬에서만 진행한다.

대신 W8 이 답하려던 질문 중 로컬에서 답할 수 있는 것을 골라 측정했다 —
"카탈로그가 붙으면 스킴 순위가 바뀌는가"([W6 보강](benchmark/w6-manifest-vs-glob.md)).
DuckDB 가 glob 대신 파일 목록을 받는다는 점을 이용해 카탈로그를 흉내냈고,
그 결과가 ADR-0004 의 기각 근거를 바로잡았다.

**답하지 못한 채 남는 것** (문서 곳곳에 "미검증 가정"으로 표시했다):

- 관리형 분산 엔진(Athena)과의 비교. 과금 모델·콜드 스타트·분산 실행
- 실 S3 의 LIST/GET 왕복 비용. 지금 결론은 MinIO 루프백 기준이다
- 실제 Iceberg 매니페스트를 읽는 비용. W6 보강은 그 비용을 뺀 **낙관적 상한**이다
