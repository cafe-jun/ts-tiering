...
3. ◐ DuckDB와 Athena 양쪽에서 같은 쿼리를 돌린 **비교 결과**가 있다 (DuckDB ✅ / Athena 는 W8)
4. ✅ ADR 4건이 작성되어 있다 (Parquet 라이브러리 / 값 타입 표현 / 쿼리 엔진 / 파티션 스킴)
...
**Exit:** ✅ 완료 (2026-08-09) — [측정](benchmark/w4-duckdb-baseline.md), ADR-0003 작성

| | 스캔 파일 | 프루닝 | 스캔량 | p50 |
|---|---|---|---|---|
| Q1 | 7 / 1,095 | 99.4% | 0.22 MiB | 1,084.9ms |
| Q2 | 365 / 1,095 | 66.7% | 11.50 MiB | 1,136.2ms |
| Q3 | 93 / 1,095 | 91.5% | 2.93 MiB | 1,071.8ms |
| glob 나열만 | — | — | 0 | 531ms |

**파티션 프루닝은 확실히 걸린다** (Q1 은 1,095개 중 7개만 연다). 그런데
**스캔량이 52배 차이인데 지연은 5% 차이다** — 531ms, 지연의 절반이 객체 나열에 들기 때문이다.

계획과 달라진 것:

- **키는 WHERE 가 아니라 glob 경로에 넣어야 한다.** PER_KEY_TYPED(ADR-0002)에서
  `value` 의 물리 타입이 키마다 달라, 키를 가로지르는 glob 은 `value` 를 VARCHAR 로 내려버린다.
  압축률 비교에서 안 드러났던 ADR-0002 의 대가가 쿼리 시점에 나타났다
...
> **W3·W4 에서 드러난 수정 사항:**
>
> - 위 표는 "경로에 무엇을 넣을지"만 변수로 두는데, 정작 크기를 2.83배 흔든 것은
>   **시간 축 granularity**(`date` vs `date/hour`)였다.
>   ADR-0004 는 스킴과 granularity 를 별개 축으로 다뤄야 한다
> - **비교 지표는 p50/p95 가 아니라 스캔 파일 수다** (W4 근거)
> - 스킴 C 는 같은 시각에 디바이스×키(255)만큼 라이터가 열리므로
>   `--max-open-writers` 를 그 이상으로 두지 않으면 축출이 파일을 쪼개 비교가 불공정해진다
> - 스킴 C 는 **나열 비용이 더 나빠진다** — 디렉터리가 디바이스 수만큼 늘어난다.
>   프루닝은 좋아지고 나열은 나빠지는 상충이라 둘을 함께 봐야 한다

- 각 스킴별로 Parquet 내부 정렬 순서도 변수로 둔다 (`deviceId,ts` vs `ts`)
- row group statistics로 프루닝이 실제로 걸리는지 확인

**Exit:** ✅ 완료 (2026-08-11) — [측정](benchmark/w5-partition-schemes.md), ADR-0004 작성

축이 하나가 아니라 넷이었다. 계획서가 예상한 것은 스킴 하나뿐이다.

| 축 | 결과 |
|---|---|
| 스킴 | C 는 바이트 26배 적고 지연 43배 느리다 — 프루닝과 지연의 승자가 정반대 |
| granularity | `hour` 는 순수 손해 (바이트 +11%, 지연 29배) |
| 파일 내 정렬 | 전송 −52%, 행 그룹 프루닝 0% → 97.7% |
| **Parquet 라이터 버전** | 계획에 없던 축. 정렬의 손익을 통째로 뒤집었다 |
...
| 쿼리 p50 / p95 | W4 ✅, W6 ✅ | 채택안 42 / 46 / 92ms | 스킴을 구분한다 — 나열 비용이 지배 |
| 쿼리당 스캔 **파일 수** | W4 ✅, W6 ✅ | 7/90 · 30/90 · 90/90 | 파티셔닝 효과의 직접 증거 |
| 쿼리당 **전송 바이트**(실측) | W6 ✅ | 0.42 / 1.50 / 7.60 MiB | 새 연결의 HTTPFS 통계 |
| 적재 처리량 (건/초) | W3 ✅ | 1,337,299 pt/s | archiver 설계 입력값 |

> **W3·W4 에서 지표 두 개를 고쳤다.**
>
> "압축률 (원본 대비)" → **"Cassandra 대비"**. NDJSON 을 분모로 쓰면 190배가 나오는데,
> Cassandra 자체가 이미 NDJSON 을 16.7배 압축하고 있어서 그 배수의 대부분은 Parquet 의 공이 아니다.
>
> "쿼리당 스캔 바이트" → **"스캔 파일 수"**. W4 에서 확인해보니 531ms 의 객체 나열 비용이
> 스킴 간 지연 차이를 전부 덮어버린다. p50/p95 로는 스킴을 구분할 수 없고,
> `EXPLAIN ANALYZE` 의 `Scanning Files: X/Y` 가 프루닝의 직접 증거다.
