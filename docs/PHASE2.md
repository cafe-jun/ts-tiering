...
2. **지연 도착이 있어도 파일 수가 통제된다** — 지연 비율별 재개봉 횟수 측정표가 있다
3. hot/cold 경계를 걸치는 시간 범위 질의가 **중복 없이** 정확한 결과를 낸다
4. archiver 를 죽였다 살려도 데이터가 유실·중복되지 않는다 (건수 대조로 확인)
5. ADR 2건 추가 (파티션 닫기 전략 / hot-cold 경계 처리)

## 주차별 계획

### W1 — 파티션 닫기 전략 (Phase 2 의 첫 관문)

Kafka 를 붙이기 전에 이것부터 푼다. 순서를 바꾸면 나중에 라이터를 다시 쓰게 된다.

- 지연 도착을 주입할 수 있게 생성기를 확장한다 (지연 비율·분포를 파라미터로)
- 전략 후보를 실제로 재본다
  - **워터마크**: 이벤트 시각이 T 를 넘으면 T 이전 파티션을 닫는다. 늦은 건 버리거나 별도 처리
  - **유예 시간**: 파티션을 닫고도 N분간 열어둔다. 메모리 대 지연의 교환
  - **재개봉 허용 후 compaction**: 일단 쪼개고 나중에 합친다 (Phase 3 의존)
- 지표: 지연 비율별 **재개봉 횟수 / 파일 수 / 유실 건수 / 메모리 사용량**

**Exit:** 지연 비율 0%·1%·5%·20% 에 대한 전략별 비교표. ADR 작성
