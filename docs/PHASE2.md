...
> **셋 다 2026-08-15 에 고쳤다** (각 항목의 '수정' 참고).
...
**수정 (2026-08-15):** `closeFile` 이 `slot.writer` 를 먼저 떼어낸 뒤 작업하고,
어떤 경로로 실패하든 `finally` 성격의 catch 에서 핸들을 닫는다.
축출·워터마크 스윕은 닫기에 성공한 뒤에 맵에서 지운다.
`close()` 는 `closed = true` 를 루프 뒤로 옮기고 `RuntimeException` 까지 잡아
하나가 실패해도 나머지를 계속 닫는다.
`write()` 에는 비어 있는 슬롯을 다시 여는 가드를 넣었다.

회귀 테스트: `WriterFailureTest` 3건. 디렉터리를 읽기 전용으로 만들어 실패를 주입한다.
특히 `writeAfterFailedRolloverReopensInsteadOfThrowingNpe` 는 **가드를 빼면 실제로 실패한다** —
반쯤 죽은 슬롯이 생기는 경로가 롤오버라는 것을 확인하는 데 시간이 걸렸다.
새 파티션은 `openFile` 실패 시 맵에 들어가기 전이라 흔적이 없다.
...
4. ◐ archiver 를 죽였다 살려도 데이터가 유실·중복되지 않는다 (건수 대조로 확인)
   — 정상 종료는 [확인](benchmark/p2w2-archiver.md). `kill -9` 는 미검증
...
**Exit:** ◐ 진행 중 — [측정](benchmark/p2w2-archiver.md)
